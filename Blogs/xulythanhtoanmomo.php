...
function execPostRequest($url, $data)
{
    ...
    // Tắt xác minh SSL (chỉ sử dụng trong môi trường phát triển)
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    
    // Thực thi yêu cầu
    $result = curl_exec($ch);
    
    // Kiểm tra lỗi từ cURL
    if(curl_errno($ch)) {
...
$amount = "1000";
$orderId = time() . "";
$redirectUrl = "http://localhost/Blogs/blog.php";
$ipnUrl = "http://localhost/Blogs/blog.php";
$extraData = "";
...
$signature = hash_hmac("sha256", $rawHash, $secretKey);  // chữ ký
